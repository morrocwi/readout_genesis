...
| v1.13 order_vacuum_selection | `order_vacuum_selection_v1_13.py` | `InfoOrderVacuumSelection.v` | **derive** `⟨H†H⟩>0` from **closure pressure** `Π_cl=Σκ_jζ_j/(1+ζ_j)`: `V_eff'(0)=α−Π_cl`, `Π_cl>α ⇒` unique `r*>0` (convex `2β>¼Σκ²`, `p(1−p)≤¼`); `a_eff=α−Π_cl<0` **derived** not inserted; RP preserved | `CLOSURE_PRESSURE_SELECTS_ORDER_EXACT` (`Th_coqc`, 8 thm) | **closes v1.12's why-order-condenses**; microscopic `g_j,Δ_j,κ_j,α,β` OPEN |
