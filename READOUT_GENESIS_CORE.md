...
precisely, a founder question raised during the 2026-07-25 matter/antimatter exploration in the
`standard_model` domain leaf ("there IS a true Record, we just cannot read it") — see
`domains/standard_model/matter_antimatter_exploration/MATTER_ANTIMATTER_EXPLORATION_LOG.md`,
Attempt 3, and `domains/standard_model/SM_INFORMATION_PHILOSOPHY_MASTER.md` §1.2 (the
domain-local restatement of the same reader-record relation `r=O(X)`). Scope: a general
foundations-level theorem about any non-injective readout, not a claim about which concrete
construction elsewhere in this canon occupies the "true state" or "record" role — that remains a
separate, case-by-case question at each face/domain where it is used.
...
## APPENDIX C (SM DOMAIN EQUATION STREAM — "INFO CORE" UNIFICATION, 2026-07-25) — carried at honest tier, provenance-tagged, NOT a new root

> **Provenance and standing rule, stated first because it is the one that matters most here.** The
> `domains/standard_model/` leaf is, by this book's own governing rule (`domains/standard_model/
> README.md` §"The one rule"), **a domain — a translation of the retained structure — never a new
> root.** Folding its equations into this Appendix is exactly that: laying the domain's own already-
> tiered results out in one continuous, numbered stream alongside ROOT-0 and Face 10 for reading
> continuity, so an AI or human reader sees the whole chain in one place without hopping repos or
> files. **It does not promote any SM-domain result to root status, does not add a new axiom to
> ROOT-0, and does not change any tier a result already carries in its own source file.** Where a
> tier says `Dr` or `finite_diagnostic` in `SM_INFORMATION_PHILOSOPHY_MASTER.md`, it says the same
> here. The only two entries below promoted to `Th_coqc` are entries that were ALREADY `Th_coqc` at
> their own source (`formal/InfoTrueRecordUnreadable_attempt.v`, merged 2026-07-25) — nothing here is
> upgraded by virtue of appearing in this Appendix.
>
> Independently peer-reviewed 2026-07-25 against live source (Coq compiles, script reruns, direct
> file reads) before being written here — 5 corrections were found and applied against a first-draft
> compiled list (see EQ-021, EQ-054, EQ-057, EQ-058, EQ-059 below for the specific fixes).
>
...
- This list is a COMPRESSED INDEX. The canonical prose earlier in this file (Part I, Face 10) and
  `SM_INFORMATION_PHILOSOPHY_MASTER.md`/the underlying `.v`/`.py` artifacts remain the full
  meaning; nothing here overrides them, and no tier here is re-derived from this list alone.
...
**Part 1 — ROOT-0 and the mother equation (already stated earlier in this file; repeated here only
as the entry point of the continuous numbered stream, EQ-001 through EQ-029 — see Part I §I.1,
§I.1a, §II.3, §II.7, §II.8, §II.8a, Face 10 above for the full prose):**
...
EQ-021  Th_coqc (abstract) / Open (concrete instantiation)
                     ...
                     𝔾_n^(-) = (𝔾_n − 𝔾_n^adjG)/2       G-adjoint, per this file's own warning]
                     ⟨z, 𝔾_n^(-) z⟩_G = 0      [notation hardened 2026-07-25: stated in the
                     retained inner product (EQ-035's ⟨x,y⟩_G) so a bare "z^T...z" is not misread
                     as a naive Euclidean bilinear — same content, disambiguated notation only]
...
EQ-026  Th_coqc (2026-07-25 grounding, see this Appendix's own Face 10 note above)
                     R_O = Γ_{RAR,O}(D_O) = Ω_A∘A∘Π∘T_Σ(D_O)
...
**Part 2 — SM domain leaf, EQ-030 through EQ-062 (`domains/standard_model/`, tiers exactly as
carried in `SM_INFORMATION_PHILOSOPHY_MASTER.md` and the domain's own exploration logs — a
translation of Part 1's root, not an extension of it):**
...
EQ-032  Th_coqc     ∀X,R,O,x1,x2 : x1≠x2 → O(x1)=O(x2) →
                     ...
                     [no_decoder_recovers_state, formal/InfoTrueRecordUnreadable_attempt.v]
EQ-033  Th_coqc     ∀X,R,O,h,x : h(x)≠x → O(h(x))=O(x) →
                     ∀D : D(O(h(x)))=h(x) → D(O(x))=x → False
                     [gauge_redundancy_forces_undecodability, same file]
EQ-034  Th_coqc     x1≠x2 → O(x1)=O(x2) →
                     (∃x:X, x=x1) ∧ (∃x:X, x=x2) ∧ ¬(∃D:R→X, D(O(x1))=x1 ∧ D(O(x2))=x2)
                     [true_state_exists_but_no_total_decoder, same file]
EQ-035  Dr          ⟨x,y⟩_G = x†Gy,  G>0
                     ‖x‖²_G = x†Gx ≥ 0

EQ-036  Th_coqc-adjacent (SM §21 closure)
                     Ω(…x_i,x_{i+1}…) = r·Ω(…x_{i+1},x_i…)
                     r² = 1 ⟹ r = ±1
                     r = −1
EQ-037  Th_coqc-adjacent (SM §21 closure)
                     Ω(x_1…x_k) = Ω(x_2…x_k,x_1)
                     (−1)^{k−1} = 1 ⟹ k odd
                     k = 3
EQ-038  Th_coqc-adjacent (SM §21 closure)
                     Ω(x,y,z) = 0 if dim V<3
                     dim V ≥ 3
                     dim V = 3,  V≅ℂ³
EQ-039  Th_coqc-adjacent (SM §21 closure)
                     U†GU = G,  G=I ⟹ U†U=I ⟹ U∈U(3)
EQ-040  Th_coqc-adjacent (SM §21 closure)
                     Ω(Ux,Uy,Uz) = det(U)·Ω(x,y,z)
                     det U = 1
                     SU(3) = {U∈U(3) : det U=1}
EQ-041  Th_coqc-adjacent (SM §21 closure)
                     9 − 1 = 8 = dim su(3)
...
EQ-049  Th_coqc-adjacent (SM §21 closure)
                     P/Q ≅ Z₃
                     τ(p,q) = p+2q (mod 3)
                     τ(3)=1,  τ(3̄)=2,  τ(8)=0
                     τ(R⊗8) = τ(R)
...
EQ-053  Th_coqc-adjacent (SM §21 closure)
                     S_p(U) = κ‖U−I‖²_F
                     ‖U−I‖² = 6−2Re Tr U
...
EQ-054  Th_coqc     c_0'(κ) = 2c_3(κ)
                     ...
                     DIFFERENT file than EQ-053's SM §11; not the same equation block in the
                     original SM text, footnoted separately per the 2026-07-25 review]
EQ-055  internal exact (SM §12)
                     P_e = ∫_𝒢 ρ_e(h)dμ(h)
                     P² = P
                     P†_G = P
                     ‖P_eT‖_G ≤ ‖T‖_G
...
EQ-065  declared_finite_architecture — primitive U/D/E branch tapes for v1.13's Π₀ (EQ-053's
        neighbor). Branch initial conditions are EXPLICITLY DISCLOSED AS ARBITRARY/uncalibrated
        (checked directly against every candidate PR in both repos — none derive them).
                     ...
                     REQUIRED CAVEAT: this Π₀ is architecturally declared, not the same
                     (independently-derived, real-PDG-mass) Π₀≈6.9888 already carried elsewhere
                     in this domain (item1_fit_calibrated_v1.py) — the two must never be conflated
                     (named Cross-Role Readout Contamination risk, this Appendix's own EQ-021
                     warning pattern repeated here for a second quantity).
...
EQ-066  declared_finite_architecture/exact_bridge — order-vacuum threshold closure. α_ord=a/2,
        β_ord=b/4 INHERITED from EQ-023's own mother potential (not new dials).
                     ...
                     unconditional lower bound. This is disclosed in the merged code's own
                     claim_boundary; do not cite ORDERED_READY as evidence the branch construction
                     is predictive.
...
EQ-068  finite_diagnostic (THREE NEGATIVE FINDINGS, all disclosed, none hidden) — 2026-07-25
        ...
        (b) SAME test with ZERO fitted parameters via the ratio m_higgs_native/v_native vs real
            m_H/v — identical 74.13% error (Λ cancels algebraically in a ratio; confirms (a)'s
            mismatch is real, not an artifact of the chosen Λ).
        (c) zero-external-input internal consistency: m_from_τc_native := 1/(2·(M_joint/D))
            (M/D used only as a declared Dr-tier ANALOGY to τ_c, not claimed identical to this
            file's own EQ-063 τ_c object) vs EQ-067's curvature-mass route — ratio 0.0612, NOT
            CONSISTENT, 94% deviation.
        OPEN QUESTION, explicitly not resolved by this Appendix: whether comparing EQ-064–067's
        native-unit outputs against real GeV values is even a valid readout-vs-readout comparison
        in this book's own sense, or a category error against a non-readout (dimensional
        constants ℏ,c,G — see this book's own companion doc `ZERO_INFINITY_DUAL_DIAGNOSIS.md`'s
        STANDING GUARD) — this tension conflicts with this domain's own routine use of PDG masses
        as fit_calibrated inputs elsewhere (e.g. EQ-058) and was found, not settled, 2026-07-25. A
        real, pre-existing, Th_coqc discrete-to-apparent-continuum bridge
        (`PERSISTENT_WALK_TELEGRAPH_DERIVATION.md`, τ_c-parametrized, "partially discharged") was
        located and is structural only — it supplies no numeric GeV conversion. An `ultracode`
