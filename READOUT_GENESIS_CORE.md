...
**Electroweak symmetry breaking — the order sector (v1.12–v1.13).** The frame-mixing arc gave a Lorentz shadow; this pair derives **electroweak symmetry breaking** on top of it, tier `Th_coqc`, still fenced. **(v1.12) Minimal Order/Higgs.** From the blind matter skeleton, the minimal order carrier is **forced**: a **color-singlet** (to preserve the SU(3) vacuum), an **SU(2) doublet** (`2⊗R₂⊃1 ⟺ R₂=2`, the only rep bridging weak-doublet→singlet), and hypercharge `y_H=3` fixed by *every* matter closure (`1+h−4=0`, `1−h+2=0`, `−3−h+6=0`) — i.e. `H=(1,2)_{1/2}`, the Higgs rep **derived, not fed**. A nonzero order `H_*` has stabilizer `Q_res=T₃+Y` (dim 1), so of the four `SU(2)×U(1)` generators `4−1=3` are broken; the neutral `(W³,B)` mass matrix `(v²/4)[[g²,−gg'],[−gg',g'²]]` has **det 0, rank 1** ⇒ one massless photon `A∝(g',g)` and `m_Z²=(g²+g'²)v²/4`, while the charged pair gives `m_W²=g²v²/4` ⇒ **`m_W=m_Z cosθ`** and tree-level **`ρ=1`**; the degree-of-freedom count balances `8+4=2+9+1`. `order_higgs_closure_v1_12.py` + `InfoOrderHiggsClosure.v` (**Closed**, 12 thm). **(v1.13) Order Vacuum Selection.** The remaining question — *why* `⟨H†H⟩>0` — is answered by **closure pressure**, not a hand-inserted negative mass. Starting from a **non-ordering** bare cost `V₀(r)=αr+βr²` (`α≥0`, `β>0`, bare minimum at `r=0`), summing the closed/open history states of the matter branches gives `V_eff(r)=αr+βr²−Σ_j log[(1+ζ_j e^{κ_j r})/(1+ζ_j)]`; the closure pressure `Π_cl=Σκ_j ζ_j/(1+ζ_j)` makes `V_eff'(0)=α−Π_cl`, so when **`Π_cl>α`** the origin loses stability and (with `V_eff→+∞` and the convexity gate `2β>¼Σκ²`, which uses `p(1−p)≤¼`) a **unique** `r*>0` is selected. The effective negative slope `a_eff=α−Π_cl<0` is thus **derived from closed-history counting**, not posited; `v²=2r*` sets the vector-mass scale (rank pattern unchanged) and reflection positivity is preserved. `order_vacuum_selection_v1_13.py` + `InfoOrderVacuumSelection.v` (**Closed**, 8 thm). **Honest status:** the electroweak *structure* (representation, stabilizer, mass rank/pattern, and the mechanism that turns order on) is derived; the *numbers* are not — the scale `v`, couplings `g,g'`, the scalar/fermion masses, and mixing all stay OPEN, as does whether `Π_cl>α` is FORCED (compute `g_j,Δ_j,κ_j,α,β` from the tape/intertwiner grammar) or merely POSSIBLE. The gauge-invariant statement is `⟨H†H⟩>0` (Elitzur). NOT a prediction of the W/Z/Higgs masses. Full ledger: `domains/standard_model/CLAIM_BOUNDARY.json`, `domains/standard_model/INDEX.md`.
